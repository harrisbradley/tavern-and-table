  ...
  return (
    <div className="absolute inset-0 z-50 flex flex-col bg-gradient-to-b from-theme-radial-start via-theme-radial-mid to-theme-radial-end overflow-y-auto">
      ...
      <div className="relative px-4 pt-8 pb-6 text-center">
        <button
          onClick={onCancel}
          className="absolute top-6 right-4 p-2 rounded-full text-slate-600 hover:text-slate-300 transition-colors"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <h2 className="text-4xl font-extrabold text-white mb-1">
          Level {character.level} → <span className="text-amber-400">{nextLevel}</span>
        </h2>
        <p className="text-slate-400 text-sm">{character.name} grows stronger.</p>
      </div>

      {/* Stat changes */}
      <div className="px-4 mb-6">
        <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-3">What changes</p>
        <div className={`grid gap-3 ${profChanged ? "grid-cols-2" : "grid-cols-1"}`}>
          {/* HP gain */}
          <div className="flex items-center gap-3 p-4 rounded-2xl bg-red-900/10 border border-red-800/30">
            <div className="w-10 h-10 rounded-xl bg-red-500/10 border border-red-500/20 flex items-center justify-center shrink-0">
              <Heart className="w-5 h-5 text-red-400" />
            </div>
            <div>
              <div className="text-red-400 text-2xl font-extrabold leading-none">+{info.hpGain}</div>
              <div className="text-slate-400 text-xs font-semibold mt-0.5">Max HP</div>
            </div>
          </div>

          {/* Proficiency bonus — only if it changes */}
          {profChanged && (
            <div className="flex items-center gap-3 p-4 rounded-2xl bg-indigo-900/10 border border-indigo-800/30">
              <div className="w-10 h-10 rounded-xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center shrink-0">
                <Shield className="w-5 h-5 text-indigo-400" />
              </div>
              <div>
                <div className="text-indigo-400 text-2xl font-extrabold leading-none">
                  +{info.oldProficiencyBonus} → +{info.newProficiencyBonus}
                </div>
                <div className="text-slate-400 text-xs font-semibold mt-0.5">Prof. Bonus</div>
              </div>
            </div>
          )}
        </div>
      ...
      {info.features.length > 0 && (
        <div className="px-4 mb-8">
          <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-3">New abilities</p>
          <div className="space-y-3">
            {info.features.map((feature) => (
              <div
                key={feature.name}
                className="flex gap-3 p-4 rounded-2xl bg-amber-900/10 border border-amber-800/20"
              >
                <div className="mt-0.5 shrink-0">
                  <Sparkles className="w-4 h-4 text-amber-400" />
                </div>
                <div>
                  <div className="text-amber-300 text-sm font-bold mb-1">{feature.name}</div>
      ...
      {info.features.length === 0 && (
        <div className="px-4 mb-8">
          <div className="p-4 rounded-2xl bg-slate-900/40 border border-slate-800 text-slate-500 text-sm text-center">
